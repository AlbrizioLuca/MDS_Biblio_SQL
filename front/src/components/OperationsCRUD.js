  ...
  if (window.confirm("Êtes-vous sûr de vouloir créer cette donnée ?")) {
    ...
    if (
      fields.every((field) => {
        ...
        const value =
          typeof newData[field.name] === "string"
            ? newData[field.name].trim()
            : newData[field.name];
  ...
  if (window.confirm("Êtes-vous sûr de vouloir modifier cette donnée ?")) {
    ...
    if (
      fields.every((field) => {
        ...
        const value =
          typeof editingData[field.name] === "string"
            ? editingData[field.name].trim()
      ...
      try {
        ...
        if (response.ok) {
          const updatedData = data.map((item) =>
            item.id === editingData.id ? cleanValue : item
          );
          setData(updatedData);
          setEditingData(null);
        }
...
export const handleDelete = async (
  ...
  if (window.confirm("Êtes-vous sûr de vouloir supprimer cette donnée ?")) {
    try {
      ...
      if (response.ok) {
        setData((data) => data.filter((item) => item.id !== selectedData.id));
