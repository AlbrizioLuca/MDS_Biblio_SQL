...
export default function SignUp() {
  const [formData, setFormData] = useState({
    // Déclaration du state formData avec les champs username, email, password et isAdmin
    username: "",
    email: "",
    password: "",
    isAdmin: false,
  ...
  const handleChange = (event) => {
    ...
    setFormData({
      // Mise à jour du state formData avec la nouvelle valeur du champ modifié
      ...formData,
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.alert(
          "Attention, veuillez remplir correctement les champs du formulaire."
        );
      }
    } catch (error) {
